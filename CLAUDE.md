...
* Deduplicating the repeated `supabase.auth.getUser()` calls across server components (e.g. by resolving the user once in the proxy and passing it down) was considered and deliberately not done: it touches the auth/session boundary and can't be verified end-to-end without a real logged-in session, which isn't available in this environment. Revisit with the real account logged in if dashboard load still feels slow after the fetch-parallelization above.
